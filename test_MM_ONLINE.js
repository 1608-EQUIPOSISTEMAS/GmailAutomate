...
const bannerId = "1KJ1l_w-WQeZ3jOuzUALhzvplZId4h3Pu";
...
const transporter48h = nodemailer.createTransport({
    ...
    auth: {
        user: "emailapikey",
        pass: "wSsVR61wrhOjCqsslTStcr86yAxUDwz2Fhwr2FCivXapHvnB88c+nkLKA1CkHPZLFmNoEmNA8O58yxgBhDMGj9UszgkHXSiF9mqRe1U4J3x17qnvhDzPXG1alhqPLIsAwg5jmGBkFc8m+g=="
    },
});

async function enviarRetiroSAPMM() {
    console.log("🚀 Iniciando envío masivo de Retiro SAP MM...");

    try {
        const templatePath = path.join(__dirname, 'template_online.html');
        const footerPath   = path.join(__dirname, 'footer.html');

        if (!fs.existsSync(templatePath)) throw new Error("Falta template_online.html en la carpeta.");

        let htmlTemplate = fs.readFileSync(templatePath, 'utf8');
        let htmlFooter   = fs.existsSync(footerPath) ? fs.readFileSync(footerPath, 'utf8') : "";

        // El HTML se prepara una sola vez (el banner es igual para todos)
